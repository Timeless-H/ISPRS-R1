...
# TRAIN_PATH = ROOT_PATH / 'subsampled/training'
...
tr_loader, val_loader = data_loaders(data_path, cfg.sampling_type,
...
model = iNet(num_layers=4, use_color=use_color, mode='unet')  # pyramid or unet
...
logging.info('Model Optimizer: %s ' % optimizer)
logging.info('Model lr_scheduler: %s' % scheduler)
...
tb = SummaryWriter(comment=f'subs_type=mine | non-pyramid | 2heads | utm_offset | fi-mha')
