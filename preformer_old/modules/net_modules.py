...
from preformer.modules.trans_modules import fast_attn, IT_Fast_Attn, sth2
from preformer.helper_files.gen_utils import poolNsample, indexing_neighbor, sth3
...
class EncoderModule(nn.Module):
    def __init__(self, d_in: int, d_out: int, depth: int=1, downsample: bool=True,
    ...
    def forward(self, x):
        ...
        for i in range(self.depth):
            ...
            x = self.block_modules[i](x)  # data --> x
        return x



class PyramidLevel1Block(nn.Module):
    def __init__(self, d_in: int, d_out: int, save_var: str,
...
class DecoderModule(nn.Module):
    ...
    def forward(self, pos, x):
        ...
        for i in range(self.depth):
            ...
            sth2.n_idx = sth.iter_set['neigh_idx'] #[POOL_CNT]
            # sth2.dist = sth.iter_set['neigh_dist'][POOL_CNT]
            x = self.block_modules[i](data)  # pos, x
        return x
    ...
    def __init__(self, num_layers: int, use_color: bool, mode: str):
        ...
        for i in range(num_layers):
            ...
            self.encoder_modules.append(
                EncoderModule(CHANNELS[i], CHANNELS[i+1],
        ...
        if self.mode == 'pyramid':
            ...
            for i in range(num_layers):
                if i == 0:
                    ...
                    upsample = True
                    # d_in = getChannelsAtIndex(i) * 1.5
                    d_in = CHANNELS[i+1] * 1.5
                self.decoder_modules.append(
                    DecoderModule(
                        int(d_in), 
                        CHANNELS[i], # getChannelsAtIndex(i-1),
